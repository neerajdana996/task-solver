...
import TagsViewer from '../shared/TagsViewer'

function classNames(...classes) {
    return classes.filter(Boolean).join(' ')
}

export default function TaskDetailHeading({ task }) {
    return (
        <div className="border-b border-gray-200 pb-5">
            <div className="sm:flex sm:items-baseline sm:justify-between">
                <div className="sm:w-0 sm:flex-1">
                    <h1 id="message-heading" className="text-base font-semibold leading-6  capitalize text-gray-900">
                        {task?.title}
                    </h1>
                    <p className="mt-1 truncate text-sm text-gray-500">Checkout and Payments Team</p>
                    <TagsViewer tags={task?.tags?.map(a => a.name) || []} />
                </div>
